...
const createNew = async (data) => {
  try {
    ...
    if (validatedData.parentId) {
      const parentComment = await GET_DB().collection(COMMENT_COLLECTION_NAME).findOne({ _id: new ObjectId(validatedData.parentId) })
      if (!parentComment) {
      ...
      await GET_DB().collection(COMMENT_COLLECTION_NAME).updateMany(
      ...
      await GET_DB().collection(COMMENT_COLLECTION_NAME).insertOne(newCommentData)
      return newCommentData

    } else {
      // If no parentId, set left and right values for root comment
      const maxRightValue = await GET_DB().collection(COMMENT_COLLECTION_NAME).findOne({ productId: validatedData.productId }, { sort: { comment_right: -1 } })
      if (maxRightValue) {
    ...
    newCommentData.comment_left = rightValue
    newCommentData.comment_right = rightValue + 1
    return await GET_DB().collection(COMMENT_COLLECTION_NAME).insertOne(newCommentData)
  } catch (error) {
...
const getCommentByParentId = async (parentId, productId) => {
  try {
    ...
    if (parentId) {
      ...
      const parentComment = await GET_DB().collection(COMMENT_COLLECTION_NAME).findOne({ _id: new ObjectId(parentId) })
      if (!parentComment) {
        throw new Error('Parent comment not found')
      }
