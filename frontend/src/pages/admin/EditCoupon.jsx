...
const EditCoupon = () => {
  ...
  useEffect(() => {
    if (coupon) {
      setFormData({
        ...
        start_day: new Date(coupon.start_day).toISOString().split("T")[0],
        end_day: new Date(coupon.end_day).toISOString().split("T")[0],
        type: coupon.type,
        value: coupon.value,
      ...
      if (coupon.target_ids) {
        coupon.target_ids.forEach((productId) => {
          initialSelected[productId] = true
        })
      }
  ...
  // Update formData.target_ids only when user explicitly changes selections
  const syncTargetIds = () => {
  ...
  // Call syncTargetIds when selectedProducts changes
  useEffect(() => {
    syncTargetIds()
  }, [selectedProducts])
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(
        "Không thể cập nhật coupon: " + (err?.data?.message || err.message)
      )
    }
  }
  ...
  if (isFetchError) {
    return (
      <div className="flex flex-col items-center justify-center h-screen text-center">
        ...
        <h3 className="text-lg font-medium">Không thể tải coupon</h3>
        <p className="text-muted-foreground mb-4">
          {fetchError?.data?.message || "Đã xảy ra lỗi"}
        </p>
        <Button onClick={() => navigate("/admin/coupons")}>Quay lại</Button>
      </div>
    )
  }
  ...
  return (
    <div className="p-6">
      <h1 className="text-3xl font-bold mb-6">Chỉnh sửa Coupon</h1>
      <Card>
        <CardHeader>
          ...
          >
            ...
            <div className="col-span-2 flex justify-end gap-4">
              ...
              <Button type="submit" disabled={isUpdating}>
                {isUpdating ? "Đang cập nhật..." : "Cập nhật Coupon"}
              </Button>
            </div>
          </form>
        </CardContent>
